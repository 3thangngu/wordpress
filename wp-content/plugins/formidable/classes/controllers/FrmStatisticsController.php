...
class FrmStatisticsController
{
    ...
    {
        $menu_label = __('Statistics', 'formidable');
        if (!FrmAppHelper::pro_is_installed()) {
            $menu_label .= ' (Lite)';
        }

        add_submenu_page('formidable', 'Formidable | ' . $menu_label, $menu_label, 'frm_view_forms', 'statistics', 'FrmStatisticsController::route');
    ...
    public static function maybe_load_listing_hooks()
    {
        ...
        add_filter('get_user_option_manageformidable_page_statisticscolumnshidden', 'FrmFormsController::hidden_columns');

        add_filter('manage_formidable_page_statistics_columns', 'FrmFormsController::get_columns', 0);
        add_filter('manage_formidable_page_statistics_sortable_columns', 'FrmFormsController::get_sortable_columns');
    }
...
//        LEFT JOIN " . $prefix . "frm_item_metas fim ON fim.field_id = ffi.id
        $query = "SELECT * FROM " . $prefix . "frm_fields ffi WHERE ffi.form_id = {$id}";
...
//                case 'text':
                    {
                        $entry_values = $wpdb->get_results("SELECT fim.meta_value FROM " . $prefix . "frm_items fi LEFT JOIN " . $prefix . "frm_item_metas fim ON fi.id = fim.item_id WHERE fi.form_id = {$id} AND fim.field_id = {$field->id}");
        ...
        require(FrmAppHelper::plugin_path() . '/classes/views/frm-statistics/show.php');
    }
    ...
    public static function bulk_trash($ids)
    {
        ...
        $message = sprintf(
            ...
            $count,
            '<a href="' . esc_url(wp_nonce_url('?page=formidable&frm_action=list&action=bulk_untrash&form_type=' . $current_page . '&item-action=' . implode(',', $ids), 'bulk-formidable_page_statistics')) . '">',
            '</a>'
        );
    ...
    {
        //check nonce url
        $permission_error = FrmAppHelper::permission_nonce_error('frm_delete_forms', '_wpnonce', 'bulk-formidable_page_statistics');
        if ($permission_error !== false) {
    ...
    public static function display_forms_list($params = array(), $message = '', $errors = array())
    {
        ...
        $wp_list_table = new FrmStatisticsListHelper(compact('params'));
    ...
    public static function process_bulk_form_actions($errors)
    {
        ...
        $permission_error = FrmAppHelper::permission_nonce_error('', '_wpnonce', 'bulk-formidable_page_statistics');
        if ($permission_error !== false) {
